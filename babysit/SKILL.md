...
2. Fetch the full diff (`gh pr diff <number>`) to understand what changes are being made.
3. Confirm the PR is not draft and inspect mergeability, checks, review decision, comments, and review threads.
4. Watch pending checks until they finish. Poll at a practical interval, usually 30-60 seconds unless the user asks for a different cadence.
5. Read new comments and unresolved review threads. Treat bot summaries as useful, but verify actionable findings against the code.
6. Fix real issues in focused commits, run relevant tests/builds, push, and return to step 2.
7. Resolve stale review threads only after verifying the code or generated artifact now addresses the comment.
8. Stop only when checks are passing or intentionally skipped, review decision is acceptable, no actionable comments remain, and no unresolved review threads remain.

## GitHub CLI Checks

Fetch the diff first to understand the scope:
```bash
gh pr diff <number>
```
...
- Always fetch `gh pr diff` at the start of each loop to understand the full scope of changes.
